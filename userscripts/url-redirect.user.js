...
// @namespace    ling
// @version      1.4
// @description  重定向 i.gkd.li 使用自定义域配置修补设备重定向,支持临时禁用跳转
...
(function () {
  ...
  // 跳转开关逻辑
  // ===============================
  const redirectEnabled = GM_getValue('redirectEnabled', true);

  GM_registerMenuCommand(
    redirectEnabled ? '❌ 临时禁用跳转' : '✅ 启用跳转',
    function () {
      GM_setValue('redirectEnabled', !redirectEnabled);
      alert(!redirectEnabled ? '跳转已启用' : '跳转已禁用（刷新页面生效）');
    },
  );

  // 如果禁用跳转，直接停止
  if (!redirectEnabled) {
    console.log('[GKD] 重定向已禁用');
    return;
  }
